...
Reconstruct* Y = new Reconstruct;
 ...
 WENO_X(U1, X->UR1,X->UL1);
 WENO_X(U2, X->UR2,X->UL2);
 WENO_X(U3, X->UR3,X->UL3);
 WENO_X(U4, X->UR4,X->UL4);
 WENO_X(U5, X->UR5,X->UL5);
 WENO_X(U6, X->UR6,X->UL6);
...
f.G4[i][j] = 0.5*(Y->UL3[i][j]*Y->UL4[i][j]/Y->UL1[i][j]+ Y->UR3[i+1][j]*Y->UR4[i+1][j]/Y->UR1[i+1][j]) -((Y->UL6[i][j]*Y->UL8[i][j])+Y->UR6[i+1][j]*Y->UR8[i+1][j])*0.5 - (Y->UR4[i+1][j]-Y->UL4[i][j])*c.ly[i][j]*0.5; 
